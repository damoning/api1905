...
class IndexController extends Controller
{
    ...
    public function doReg()
    {
        ...
        //请求passport注册
        $url = 'http://passport.1905.com/api/index/reg';
        $client = new Client();
    ...
    public function doLogin()
    {
        unset($_POST['_token']);
        //echo '<pre>';print_r($_POST);echo '</pre>';
        $url = 'http://1905passport.com/sign/index/login';
        $client = new Client();
        $response = $client->request('post', $url, [
            'form_params' => $_POST
        ]);
    ...
    public function center()
    {
        ...
        if (empty($token) || empty($uid)) {
            header('Refresh:2;url=/index/login');        //两秒跳转
            echo "请先登录, 页面跳转中";
            die;
        }
        //请求passport 鉴权
        $url = 'http://passport.com/api/auth';
        $client = new Client();
        $response = $client->request('post', $url, [
